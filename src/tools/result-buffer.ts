...
export class ToolResultBuffer {
  ...
  maybeBuffer(result: string, toolName: string): string {
    ...
    if (result.length <= this.threshold) return result;

    return this.bufferToDisk(result, toolName);
  }
  ...
  private buildPointerMessage(
    ...
    parts.push(
      '',
      'Use read(file_path="' + filePath + '", offset=1, limit=2000) to view in segments.',
    );
