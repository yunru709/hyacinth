 ...
 * 判据（都对着真实产出，不看文档）：
 *  ① .ts（ts-ast）/ .py（py-tree-sitter）/ .go（go-tree-sitter）/ .rs（rust-tree-sitter）→ [precise]；
 *     .kt（generic-regex）→ [heuristic] —— 代表刻意选 **Kotlin**：任务单明确 kotlin/swift
 *     维持正则兜底不动，故它不会再被升级打脸（前两次用 .go、.rs 都被升级顶掉）
...
describe('Phase 3：置信度标注', () => {
  ...
  it('① 语义链语言（ts/py/go/rs）→ [precise]；仍走正则的（kt）→ [heuristic]', async () => {
