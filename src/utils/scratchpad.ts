 ...
 *   · 用途（2026-09-19 用户定）：**我随手记东西的地方** ——
 *     ① 新东西**先记这儿**（别直接往 memory 塞 ✗）；② **每次写入顺手整理**；
 *     ③ 快满时审一遍 ⇒ 真正重要的**精炼**成 memory 条目，其余丢弃 ✓。
 *
...
export function scratchpadSeed(): string {
  return [
    ...
    '',
    '<!-- 用途（2026-09-19 用户定）：**我随手记东西的地方** ——',
    '     ① 有新东西**先记这儿**（别直接往 memory 塞 ✗）；',
    '     ② **每次写入顺手整理**（合并同类、删掉过期的）；',
    '     ③ **快满时审一遍**：真正重要的**精炼**成 memory 条目，其余丢弃 ✓。',
    '     性质：每轮现读现注入、**不进消息历史**；比 memory 更临时，可随手改。',
    '     上限：默认 8000 字符（context.scratchpadMaxChars），超出会截断并标注。',
    '-->',
    '',
  ].join('\n');
}
