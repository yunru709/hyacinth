...
describe('ToolRegistry overwriteGuard', () => {
  it('插件来源不可覆盖内置工具', () => {
    ...
    expect(registry.get('bash')!.execute({}, undefined)).resolves.toBe('original');
  });
  ...
  it('MCP 来源不可覆盖内置工具', () => {
    ...
    expect(registry.get('write')!.execute({}, undefined)).resolves.toBe('original');
  });

  it('同源重注册允许（MCP 重连 / 插件热重载场景）', () => {
    const registry = new ToolRegistry();
    registry.register(stub('mcp__srv__tool', 'v1', 'mcp'));
    registry.register(stub('mcp__srv__tool', 'v2', 'mcp'));
    expect(registry.get('mcp__srv__tool')!.execute({}, undefined)).resolves.toBe('v2');
  });

  it('无来源（core）工具之间覆盖仍允许（重启/重复装配场景）', () => {
    const registry = new ToolRegistry();
    registry.register(stub('read', 'v1'));
    registry.register(stub('read', 'v2'));
    expect(registry.get('read')!.execute({}, undefined)).resolves.toBe('v2');
  });
});
