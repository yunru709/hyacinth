...
export async function runCli(): Promise<void> {
  ...
  async function runServer(options: Record<string, string>, webuiEnabled: boolean): Promise<void> {
    ...
    if (webuiEnabled) {
      // 对外开放时，127.0.0.1 对"别的设备"没用 ✗ ⇒ 打开本机内网地址（本机自己访问同样有效 ✓）
      ...
      if (options.lan || options.host) {
        try {
          ...
          if (urls.length > 0) openUrl = urls[0]!.split('  ←')[0]!.trim();
        } catch { /* 取不到就用本机地址 */ }
      }
      openBrowser(openUrl);
    }
...
// ── [圈三锚点 · 展示位] ──────────────────────────────────────────────
// 第三圈（联动清单 tool-links.json）接入时，改动落在这里：links 段在此渲染；第三圈接入后改为"声明态 + 运行态"两栏（数据源同此）
