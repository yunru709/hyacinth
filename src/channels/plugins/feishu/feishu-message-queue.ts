// ============================================================
// FeishuMessageQueue — 飞书渠道 per-session 消息队列
// ============================================================
//
// 解决的问题：
//   1. SDK handler 阻塞 ACK → 飞书重传 → 重复消息
//   2. 并发 loop.run() → conversation.jsonl 交叉写入
//
// 设计：
//   - handler 收到消息后入队即返回（fire-and-forget），ACK 立即发出
//   - 队列按 sessionId 串行消费，保证同一 session 不会并发处理
//   - 队列内二次去重，防止重传消息漏过外层去重窗口
...
export class FeishuMessageQueue {
  ...
  constructor(logger?: Logger) {
    this.logger = logger ?? {
      info: (msg) => process.stderr.write(`[feishu-queue] ${msg}\n`),
      error: (msg) => process.stderr.write(`[feishu-queue] ${msg}\n`),
      warn: (msg) => process.stderr.write(`[feishu-queue] ${msg}\n`),
    };
  }
  ...
  enqueue(sessionId: string, event: ChannelEvent): void {
    ...
    if (queue.length >= FeishuMessageQueue.MAX_QUEUE_SIZE) {
      const dropped = queue.shift();
      const droppedId = getMessageId(dropped?.event ?? {} as ChannelEvent);
      (this.logger.warn ?? this.logger.error)(`queue overflow: dropping oldest message ${droppedId?.slice(0, 12) ?? 'unknown'}...`);
    }
  ...
  private processNext(sessionId: string): void {
    ...
    this.processFn(item.event)
      .catch((err) => {
        this.logger.error(`process error for session ${sessionId.slice(0, 16)}...: ${String(err instanceof Error ? err.message : err)}`);
      })
      .finally(() => {
